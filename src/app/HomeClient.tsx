...
export function HomeClient({ initialTracks }: HomeClientProps) {
  const { data: tracks, isLoading, error } = useListTracks(initialTracks);
  ...
  // Seeds the track-by-slug cache with the tracks we already have, so
  // clicking into one of these from the homepage lands on an already-warm
  // detail page instead of showing a loading state again.
  const queryClient = useQueryClient();
  const seededSlugs = useRef(new Set<string>());
  useEffect(() => {
    for (const track of initialTracks) {
      if (seededSlugs.current.has(track.slug)) continue;
      seededSlugs.current.add(track.slug);
